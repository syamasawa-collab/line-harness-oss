...
export function registerManageFriends(server: McpServer): void {
  server.tool(
    "manage_friends",
    "友だちの管理操作。count: 友だち数取得、set_metadata: メタデータ更新（デフォルトは浅いマージ。replace=true で全置換 — キー削除は全置換でのみ可能）、set_rich_menu: リッチメニュー割当、remove_rich_menu: リッチメニュー解除。",
    {
      action: z
      ...
      replace: z
        ...
        .optional()
        .describe(
          "For 'set_metadata': true replaces the ENTIRE metadata object with the given JSON (allows deleting keys). Default false = shallow merge.",
        ),
      richMenuId: z
        .string()
        .optional()
    ...
    async ({ action, friendId, metadata, replace, richMenuId }) => {
      try {
